...
function runConfig(cmdWithoutHidden, cmdWithHidden, cmdOnlyOne, cmdHiddenOnlyOne) {
  try {
    ...
    if (!(0, _io.hasFiles)(__dirname + '/tests_written/*.js')) {
      console.log(_colors2.default.red('Error: There is no test files or valid test files'));
      process.exit();
    }
...
function start(cmd1, cmd2, cmd3, cmd4) {
  _commander2.default.version('0.8.0').option('-c, --config <config>', 'read config file').option('-o, --omit', 'omit browser window(real-test needs xvfb)').option('-t, --test <test>', 'only runs on test').parse(process.argv);
